...
export default function PathologyEmergencyOrdersPage() {
  ...
  const orders = PATHOLOGY_EMERGENCY_ORDERS;
    ...
    },
    {
      ...
      cell: (order) => (
        <div className="text-right">
          <OpsActionButton label="View Details" icon={Eye} onClick={() => router.push(`/lab/pathology/emergency-orders/${order.id}`)} className="border-violet-200 text-violet-700" />
        </div>
      ),
    },
  ];
...
action={{
          label: "Process Tests",
          icon: Eye,
          onClick: () => router.push(`/lab/pathology/emergency-orders/${order.id}`),
        }}
      />
    );
  }
