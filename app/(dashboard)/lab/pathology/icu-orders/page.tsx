...
import {
  PATHOLOGY_ICU_DOCTORS, PATHOLOGY_ICU_ORDERS,
...
export default function PathologyICUOrdersPage() {
  ...
  const orders = PATHOLOGY_ICU_ORDERS;
...
function updateFilter<K extends keyof PathologyIpdOrderFilters>(key: K, value: PathologyIpdOrderFilters[K]) {
  ...
  const infoCards: KpiCardProps[] = [
    ...
    { label: "Total ICU Orders", value: String(stats.totalOrders), icon: ReceiptText, accent: "blue", footer: `${stats.totalTests} ordered tests`, trend: buildTrend(stats.totalOrders, previousDay.totalOrders) },
    ...
    },
    {
      ...
      cell: (order) => (
        <div className="text-right">
          <OpsActionButton label="View Details" icon={Eye} onClick={() => router.push(`/lab/pathology/icu-orders/${order.id}`)} className="border-violet-200 text-violet-700" />
        </div>
      ),
    },
  ];
...
action={{
          label: "Process Tests",
          icon: Eye,
          onClick: () => router.push(`/lab/pathology/icu-orders/${order.id}`),
        }}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen">
      <PageShellHeader
        title="Pathology ICU Orders"
        description="Process ICU-based diagnostic tests, finalize reports, and manage direct or department billing."
        meta={<span className="rounded-full border border-violet-200 bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-700">Critical Care Queue</span>}
        actions={
            ...
            filters={[
              {
                ...
                options: [{ value: "", label: "All Doctors" }, ...PATHOLOGY_ICU_DOCTORS.map((d) => ({ value: d, label: d }))],
              },
              {
                key: "status",
                label: "Status",
