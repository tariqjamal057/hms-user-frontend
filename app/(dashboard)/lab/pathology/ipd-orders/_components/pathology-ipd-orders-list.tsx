// app/(dashboard)/lab/pathology/ipd-orders/_components/pathology-ipd-orders-list.tsx
"use client";
...
import { IpdPaymentBadge, UrgencyBadge } from "./pathology-ipd-badges";

interface Props {
  orders: PathologyIpdOrder[];
  onView: (order: PathologyIpdOrder) => void;
}

export function PathologyIpdOrdersList({ orders, onView }: Props) {
  return (
    <Card className="overflow-hidden border-slate-200 shadow-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {orders.map((order) => (
              <tr key={order.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-5 py-4">
                  <UrgencyBadge
                    urgency={hasUrgentTest(order) ? "Urgent" : "Routine"}
                  />
                </td>
                <td className="px-5 py-4 text-sm font-bold text-slate-800">
                  ₹{getTotalIpdTestValue(order)}
                </td>
                <td className="px-5 py-4">
      ...
      {orders.length === 0 && (
        <CardContent className="py-16 text-center text-sm text-slate-400">
          No pathology IPD orders match the selected filters.
        </CardContent>
      )}
    </Card>
  );
...
export function PathologyIpdOrdersGrid({ orders, onView }: Props) {
  return (
      ...
      {orders.map((order) => (
        ...
        >
          ...
          <CardContent className="p-5">
            ...
            <Button
              className="mt-4 w-full border-violet-200 text-violet-700"
              variant="outline"
