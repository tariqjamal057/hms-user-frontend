// app/(dashboard)/lab/radiology/icu-orders/page.tsx
"use client";
...
import {
  RADIOLOGY_ICU_DOCTORS,
  RADIOLOGY_ICU_ORDERS,
  getRadiologyIpdAggregateStatus,
  getTotalRadiologyIpdValue,
  hasUrgentRadiologyIpdTest,
} from "@/lib/lab/radiology/radiology-icu-orders-data";
...
export default function RadiologyICUOrdersPage() {
  const router = useRouter();
  const orders = RADIOLOGY_ICU_ORDERS;
...
function updateFilter<K extends keyof RadiologyIpdOrderFilters>(
  ...
  const infoCards: KpiCardProps[] = [
    ...
    { label: "Total ICU Orders", value: String(stats.totalOrders), icon: ReceiptText, accent: "blue", footer: `${stats.tests} imaging tests`, trend: buildTrend(stats.totalOrders, previousDay.totalOrders) },
    ...
    },
    {
      ...
      cell: (order) => (
        <div className="text-right">
          <OpsActionButton label="View Details" icon={Eye} onClick={() => router.push(`/lab/radiology/icu-orders/${order.id}`)} className="border-sky-200 text-sky-700" />
        </div>
      ),
    },
  ];
...
action={{
          ...
          icon: Eye,
          onClick: () => router.push(`/lab/radiology/icu-orders/${order.id}`),
        }}
      />
    );
  }
  ...
  return (
    <div className="min-h-screen">
      <PageShellHeader
        title="Radiology ICU Orders"
        description="Process ICU imaging orders, upload reports, and manage direct or billing-department payments."
        meta={<span className="rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-xs font-semibold text-sky-700">ICU Queue</span>}
        actions={
            ...
            filters={[
              {
                ...
                options: [{ value: "", label: "All Doctors" }, ...RADIOLOGY_ICU_DOCTORS.map((d) => ({ value: d, label: d }))],
              },
              {
                key: "status",
                label: "Status",
