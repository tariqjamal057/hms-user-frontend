...
export default function RadiologyICUOrdersPage() {
  const [orders, setOrders] = useState<RadiologyIpdOrder[]>(RADIOLOGY_ICU_ORDERS);
  ...
  function sendToBillingDept(orderId: string) {
    ...
    setSelectedOrder(null);
    toast.success(
      `Radiology reports delivered for ${current?.patient.name ?? "patient"}. Bill sent to ICU Billing Department.`,
    );
  }
