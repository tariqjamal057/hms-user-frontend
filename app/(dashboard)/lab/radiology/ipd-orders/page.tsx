...
export default function RadiologyIpdOrdersPage() {
  ...
  const orders = RADIOLOGY_IPD_ORDERS;
    ...
    },
    {
      ...
      cell: (order) => (
        <div className="text-right">
          <OpsActionButton label="View Details" icon={Eye} onClick={() => router.push(`/lab/radiology/ipd-orders/${order.id}`)} className="border-sky-200 text-sky-700" />
        </div>
      ),
    },
  ];
...
action={{
          ...
          icon: Eye,
          onClick: () => router.push(`/lab/radiology/ipd-orders/${order.id}`),
        }}
      />
    );
  }
