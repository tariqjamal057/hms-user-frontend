...
const ROUTE_OPTIONS: { value: FluidRoute; label: FluidRoute }[] = [
  { value: "IV", label: "IV" },
...
export function TabFluidBalance({
  entries,
  onAddEntry,
}: {
  entries: FluidBalanceEntry[];
  onAddEntry: (entry: FluidBalanceEntry) => void;
}) {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {entries.length} entries · net{" "}
              <span
                className={
                  netBalance >= 0 ? "font-semibold text-emerald-700" : "font-semibold text-rose-700"
                }
              >
                {netBalance >= 0 ? "+" : ""}
                {netBalance} ml
...
function AddFluidDrawer({
  ...
  return (
    <ConsultationDrawer
      ...
      description={`Recording as ${CURRENT_NURSE.name} (${CURRENT_NURSE.shift})`}
    ...
    >
      ...
      >
        <RadioGroup
          ...
          value={direction}
          onChange={(v) => {
            const next = v as FluidDirection;
            setDirection(next);
      ...
      >
        <SingleSelect
          label="Route"
          value={route}
          onChange={(v) => setRoute(v as FluidRoute)}
