// app/(dashboard)/pharmacy/emergency/orders/[orderId]/page.tsx
"use client";
...
import { TabTodaysOrders } from "../../../ipd/orders/_components/tab-todays-orders";
import { TabPreviousDays } from "../../../ipd/orders/_components/tab-previous-days";
import { TabReturns } from "../../../ipd/orders/_components/tab-returns";
import { TabBilling } from "../../../ipd/orders/_components/tab-billing";
import { TabAudit } from "../../../ipd/orders/_components/tab-audit";

export default function PharmacyEmergencyOrderDetailPage() {
  const params = useParams<{ orderId: string }>();
  const sourceOrder = PHARMACY_EMERGENCY_ORDERS.find((o) => o.id === params.orderId);
  const [order, setOrder] = useState<PharmacyIpdOrder>(() => sourceOrder ?? PHARMACY_EMERGENCY_ORDERS[0]);
  ...
  function handleSendToBillingDept() {
    ...
    toast.success(`Bill sent to Billing Department for ${order.patientName}.`);
  }
  ...
  if (order.department) infoFields.push({ label: "Incident Type", value: order.department });
  if (order.orderingDoctor) infoFields.push({ label: "Doctor", value: order.orderingDoctor });
  if (order.admissionDate) infoFields.push({ label: "Registered On", value: order.admissionDate });
  ...
  const detail: PatientDetailData = {
    ...
    locationParts: [order.ward, order.bed].filter(Boolean),
    metaLine: order.ipdId,
    fallbackInfoFields: infoFields,
  };

  const patientList: PatientListItem[] = PHARMACY_EMERGENCY_ORDERS.map((o) => ({
    uhid: o.uhid,
    name: o.patientName,
  ...
  return (
    <PatientDetailShell
      ...
      patientsPath="/pharmacy/emergency/orders"
      tabs={tabs}
      defaultTab="today"
    />
  );
