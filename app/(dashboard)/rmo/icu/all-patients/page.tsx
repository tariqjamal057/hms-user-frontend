// app/(dashboard)/rmo/icu/patients/page.tsx
"use client";
...
export default function RmoIcuPatientsPage() {
  ...
  function viewPatient(patient: NurseIpdPatient) {
    router.push(`/rmo/icu/all-patients/${patient.uhid}`);
  }
  ...
  const infoCards: KpiCardProps[] = [
    {
      ...
      icon: Users,
      accent: "blue",
      footer: "Under your care this shift",
    ...
    },
    {
      ...
      icon: AlertTriangle,
      accent: "rose",
      footer: "Require close monitoring",
    ...
    },
    {
      ...
      icon: HeartPulse,
      accent: "amber",
      footer: "Medicines due to be given",
      trend: buildTrend(stats.pendingDoses, previousDay.pendingDoses),
    },
    {
      label: "Out of Stock",
      ...
      icon: PackageX,
      accent: "violet",
      footer: "Doses awaiting pharmacy stock",
      trend: buildTrend(stats.outOfStock, previousDay.outOfStock),
    },
  ];
  ...
  const columns: OpsColumn<NurseIpdPatient>[] = [
    ...
    },
    {
      ...
      header: "Doctor",
      cell: (p) => (
        <span className="text-sm text-slate-600">{p.admittingDoctor}</span>
      ),
    },
    {
      key: "pendingDoses",
      ...
      cell: (p) => {
        const pending = getEmarForPatient(p.uhid).filter(
          (dose) => dose.status === "Pending",
        ).length;
        return (
          <span
            className={`text-sm font-semibold ${pending > 0 ? "text-amber-600" : "text-emerald-600"}`}
          >
            {pending}
          </span>
        );
        ...
        <CardContent className="p-5">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg border border-slate-100 p-3">
              ...
              <p
                className={`mt-1 text-sm font-bold ${pending > 0 ? "text-amber-600" : "text-emerald-600"}`}
              >
                {pending}
              </p>
            </div>
          ...
          <FilterBar
            ...
            onFilterChange={(key, value) => {
              ...
              if (key === "acuity")
                updateFilter(
                  "acuity",
                  value as NurseIpdPatientFilters["acuity"],
                );
              if (key === "shift")
                updateFilter(
                  "shift",
                  value as NurseIpdPatientFilters["shift"],
                );
            }}
          />
        </div>
