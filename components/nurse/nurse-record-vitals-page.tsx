// components/nurse/nurse-record-vitals-page.tsx
"use client";
...
import type { NurseIpdPatient, VitalRecord } from "@/types/nurse/ipd/nurse-ipd-types";
...
type Props = {
  ...
  backPath: string;
  subtitle: string;
  getVitals: (uhid: string) => VitalRecord[];
  saveVital: (vital: VitalRecord) => void;
  recordPathPrefix?: string;
};

export function NurseRecordVitalsPage({
  patient,
  patientList,
  patientsPath,
  backPath,
  ...
  getVitals,
  saveVital,
  recordPathPrefix = "/nurse/ipd/patients",
}: Props) {
  ...
  function handleDraft(data: VitalsFormData) {
    console.log("Vitals draft:", { uhid: patient.uhid, ...data });
  ...
  return (
    <div className="bg-white">
      <PatientDetailShell
        ...
        hideSingleTab
        showPatientSwitcher
        onSwitchPatient={(u) => router.replace(`${recordPathPrefix}/${u}/record-vitals`)}
        ...
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Recording as <span className="font-semibold">{CURRENT_NURSE.name}</span> ({CURRENT_NURSE.shift}).
              Review abnormal alerts and the previous vitals comparison before saving.
            </p>
          </div>
        </div>
...
function toVitalRecordEntry(v: VitalRecord): VitalRecordEntry {
  return {
    dateTime: v.dateTime,
    bp: v.bp,
    systolic: v.systolic,
...
function toWardRoundPatient(p: NurseIpdPatient, latest?: VitalRecord): WardRoundPatient {
